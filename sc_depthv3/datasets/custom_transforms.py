...
class Compose(object):
    "first RescaleTo training size; second ArrayToTensor; third Normalize"
...
class desize(object):
    """Rescale images to original size + tensor to array"""
    ...
    def __call__(self, images):
        ...
        if in_h != out_h or in_w != out_w:
            ...
            for im in images:
                ...
                descaled_images.append(img)
        else:
            descaled_images = images

        descaled_images = torch.tensor(descaled_images)
        descaled_images = torch.squeeze(descaled_images, dim=0)

        return descaled_images
    ...
    def __call__(self):
        ...
        for im in self.images:
            ...
            if im.ndim < 3:  # depth
                ...
                arrays.append(im.numpy().float())
            else:
                im = np.transpose(im, (0, 1, 2))
                arrays.append(im.numpy().float() * 255)
        return arrays